...
from create_coarse_sentences import Node, get_t5_opposite, get_caption_tree4, get_caption_tree6, get_caption_tree6_1, get_caption_tree6_2, get_caption_tree8, get_caption_tree8_1, expand_caption, get_box_text, plotly_plot_tree
...
class DecisionTreeClipModel(nn.Module):
  ...
  def get_caption_tree_figure(self, caption, file_name="tree_fig.png"):
    root, true_label_path, all_tree_nodes, edges, node_texts = get_caption_tree8_1(caption)
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  #checkpoint trained with LR_1e-6 LoRA rank = 4, get_caption_tree8_1(flan T5 opposites, T5 masks, adpositions_list_2 and co-hyponyms) for 1 epoch 
  #and then get_caption_tree6(flan T5) with prob 0.7, get_caption_tree8_1 with prob 0.2 and get_caption_tree4 with prob 0.1  
  #0.5 * DT loss  + 0.5 * contrastive loss (tree_loss_weight = 0.5)
  PATH = '/mnt5/nir/CLIP/interpret/DT_0.5_mix_t8_1_t6_t4_LoRA_4_contrast_LR_1e6_checkpoint_epoch_0008.pt.tar'
  ...
  print(f'train with lora_rank: {DT_CLIP_model.lora}, tree_loss_weight: {tree_loss_weight}, mix epoch_0 - get_caption_tree8_1: flan t5 opposites if exist otherwise fill mask otherwise co-hyponym')
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    for batch_idx, (full_path, images, captions) in enumerate(tqdm(data_loader)):
      ...
      with torch.no_grad():
        ...
        tree_func = get_caption_tree8_1 
        
        if epoch > 0:
          p = random.random()
          if p < 0.1:
            tree_func = get_caption_tree4
          elif p > 0.3:
