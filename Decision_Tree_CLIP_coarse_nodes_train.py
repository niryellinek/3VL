...
class ClipLoss(nn.Module):
    ...
    def __init__(self, use_open_clip_loss=False,
            use_non_symmetric_loss=True,
            local_loss=False,
            gather_with_grad=False,
            cache_labels=False,
            rank=0,
        ...
        self.use_open_clip_loss = use_open_clip_loss
        self.use_non_symmetric_loss = use_non_symmetric_loss
    ...
    def openclip_forward(self, image_features, text_features, logit_scale):
    ...
    def forward(self, logits_per_image, logits_per_text, logit_scale):
        
        device = logits_per_image.device

        #if we use openclip loss, logits_per_image, logits_per_text are actually image_features, text_features
        if self.use_open_clip_loss:
            logits_per_image, logits_per_text = self.openclip_forward(logits_per_image, logits_per_text, logit_scale)
        
        # calculated ground-truth and cache if enabled
        num_logits = logits_per_image.shape[0]
        num_texts = logits_per_text.shape[0]
        ...
        if self.use_non_symmetric_loss:
            total_loss = F.cross_entropy(logits_per_image, labels)
        
        elif num_logits != num_texts:
            #use only texts that have matching images
            total_loss = (
                F.cross_entropy(logits_per_image, labels) +
                F.cross_entropy(logits_per_text[0:num_logits], labels)
            ) / 2
...
class DecisionTreeClipModel(nn.Module):
  def __init__(self, clip_model, hier):
    ...
    self.coarse_label_path_dict = dict()
  ...
  def calc_label_path_dict(self):
    ...
    for class_num in range(num_imagenet_classes):
      ...
      if path:
        ...
        self.coarse_label_path_dict[class_num] = path[0:-1]

  

  def get_encoded_prompts(self, include_leaves=True):
  ...
  def duplicate_matrix_rows(self, x, labels):
    ...
    res = torch.cat([x[idx].repeat(len(self.coarse_label_path_dict[l.item()]),1) for idx, l in enumerate(labels)] )
  ...
  def get_unrolled_node_id_labels(self, labels):
    ...
    unrolled_labels = np.concatenate([np.array(self.coarse_label_path_dict[l.item()]) for l in labels ])
    #print(f'\n\n\nunrolled_labels: \n\n{unrolled_labels}')
  ...
  def get_parent_prompt_encoding_matrix(self, labels):
    
    #mask = torch.zeros_like(similarity_matrix)
    encodings_matrix = torch.zeros(len(labels),self.encoded_prompts.size(1))
    
    for idx, label in enumerate(labels):
      node_id = self.hier.LEAF_NUM_TO_ID[label.item()]
      parent_node_id = self.get_parent(node_id)
      prompt_idx = self.hier.HIER_NODE_ID_TO_NUM[parent_node_id]
      
      encodings_matrix[idx] = self.encoded_prompts[prompt_idx]
...
def angular_normalization(v, class_weights):
  v = nn.functional.normalize(nn.functional.normalize(v) - nn.functional.normalize(class_weights))
  #v = nn.functional.normalize(v)
  return v

def DT_CLIP_leaves_train(DT_CLIP_model, frozen_CLIP, num_epochs, data_loader, criterion, optimizer, save_frequency=5, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  DT_CLIP_model.train()
  loaded_epoch = 0
  #loss = None
  ...
  PATH = '/mnt5/nir/CLIP/interpret/DT_coarse_CLIP_checkpoint_epoch_0003.pt.tar'
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    for batch_idx, (images, labels) in enumerate(tqdm(data_loader)):
      ...
      prompt_encoding_matrix = DT_CLIP_model.get_parent_prompt_encoding_matrix(labels)
      prompt_encoding_matrix = prompt_encoding_matrix.to(device=device)
      
      trained_image_features = DT_CLIP_model.clip_model.encode_image(images)
      #trained_image_features = trained_image_features / trained_image_features.norm(dim=-1, keepdim=True)

      frozen_image_features = frozen_CLIP.encode_image(images)
      #frozen_image_features = frozen_image_features / frozen_image_features.norm(dim=-1, keepdim=True)

      #print(f'prompt_encoding_matrix.shape: {prompt_encoding_matrix.shape}, trained_image_features.shape: {trained_image_features.shape}')

      #perform angular_normalization on image_features
      angular_trained_image_features = angular_normalization(trained_image_features, prompt_encoding_matrix)
      angular_frozen_image_features = angular_normalization(frozen_image_features, prompt_encoding_matrix)

      
      #loss = criterion(trained_image_features, frozen_image_features)
      #loss = criterion(angular_trained_image_features, angular_frozen_image_features)
      loss = criterion(trained_image_features, frozen_image_features, frozen_CLIP.logit_scale)
      #distilled_loss = distillation_loss(text_features, distilled_text_features, frozen_model.logit_scale)
...
def main():
    ...
    frozen_CLIP, _ = clip.load('ViT-B/32', device=device)

    #freeze self.clip_model params
    for param in frozen_CLIP.parameters():
        param.requires_grad = False
        param.data = param.data.float()
        #if param.grad:
        #  param.grad.data = param.grad.data.float() 
    ...
    #leaves_criterion = nn.L1Loss()
    leaves_criterion = nn.MSELoss()
    distillation_loss = ClipLoss(use_open_clip_loss=True)#must use features and not logits
    ...
    #print(f'starting leaves training loop on train dataset')
    #DT_CLIP_leaves_train(DT_CLIP_model, frozen_CLIP, num_epochs, train_loader, leaves_criterion, optimizer, save_frequency, checkpoint_path)
    #DT_CLIP_leaves_train(DT_CLIP_model, frozen_CLIP, num_epochs, train_loader, distillation_loss, optimizer, save_frequency, checkpoint_path)
    ...
    #PATH = '/mnt5/nir/CLIP/interpret/DT_CLIP_checkpoint_epoch_0003.pt.tar'
    #PATH = '/mnt5/nir/CLIP/interpret/DT_coarse_CLIP_checkpoint_epoch_0003.pt.tar'
    #PATH = '/mnt5/nir/CLIP/interpret/DT_angular_leaves_CLIP_checkpoint_epoch_0003.pt.tar'
    #PATH = '/mnt5/nir/CLIP/interpret/DT_L1_leaves_CLIP_checkpoint_epoch_0003.pt.tar'
    #PATH = '/mnt5/nir/CLIP/interpret/DT_angular_leaves_CLIP_checkpoint_epoch_0001.pt.tar'
    #PATH = '/mnt5/nir/CLIP/interpret/DT_contrastive_distance_MSE_leaves_CLIP_checkpoint_epoch_0001.pt.tar'
    PATH = '/mnt5/nir/CLIP/interpret/DT_angular_MSE_leaves_CLIP_checkpoint_epoch_0001.pt.tar'    
