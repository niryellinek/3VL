...
def main():
    ...
    img_embeds = img_embeds.to(device=device)
    attribute_num_appearances = dict()
    for idx, prompt in enumerate(tqdm(encoded_prompts)):
        ...
        apperances = similarity[similarity >= apperances_threshold]
    ...
    with open(f"attribute_num_appearances_threshold_{apperances_threshold}.json", "w") as fp:
        json.dump(attribute_num_appearances, fp)
