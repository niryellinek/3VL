...
def get_tree_loss(model, tree_criterion, images, captions, dataset_index, tree_func, weights_list):
  
  with torch.no_grad(): 
    
    batch_weights = weights_list[dataset_index].sum()
  ...
  dataset_sample_index = dataset_index[rand_sample_index]
  weighting = weights_list[dataset_sample_index]
  tree_loss = tree_criterion(log_probabilities, node_num_labels, weighting)

  # normilize by the sum of all weights in the batch
  tree_loss = tree_loss.sum() * 1 / batch_weights
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  weight_file = "CC3M_samples_weighting.json"
  print(f'loading samples weight_file: {weight_file}')

  with open(weight_file, "r") as f:
    weights_list = json.load(f)

  # weights_list = np.array(weights_list)
  weights_list = torch.tensor(weights_list, dtype=torch.float)
  ...
  for epoch in range(loaded_epoch,num_epochs):
    print(f'start calculating samples weights epoch#: {epoch+1}')
    ...
    for batch_idx, (index, images, captions, image_names) in enumerate(tqdm(data_loader)):
      ...
      tree_loss, rand_sample_index = get_tree_loss(DT_CLIP_model, tree_criterion, images, captions, index, tree_func, weights_list)

      if not tree_loss:
        print(f'tree_loss is None. continue to next batch')
        continue
      ...
      loss_img = img_criterion(logits_per_image, ground_truth, weights_list[index])
      loss_txt = txt_criterion(logits_per_text, ground_truth, weights_list[index])
      contrastive_loss = (loss_img + loss_txt)/2

      # normilize by the sum of all weights in the batch
      contrastive_loss = contrastive_loss.sum() * 1 / weights_list[index].sum()
      #average by num ff examples in contrastive batch
      contrastive_loss = contrastive_loss / num_examples
...
def main():
    ...
    tree_criterion = weighted_tree_loss

    # Contrastive Loss 
    # img_criterion = nn.CrossEntropyLoss(reduction='none')
    # txt_criterion = nn.CrossEntropyLoss(reduction='none')
    img_criterion = weighted_contrastive_loss
    txt_criterion = weighted_contrastive_loss
...
def weighted_tree_loss(y, y_hat, weighting):
      part = nn.NLLLoss(reduction='none')
      loss = part(y, y_hat)
      return loss * weighting


def weighted_contrastive_loss(y, y_hat, weighting):
      part = nn.CrossEntropyLoss(reduction='none')
      loss = part(y, y_hat)
      return loss.cpu() * weighting
