...
def main():
    ...
    print(f'use (positive + negative)/2 minus anchor relevancy map')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_average_minus_anchor_cam for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    cam_image = (positive_cam_image + negative_cam_image)/2 - ancor_cam_image
