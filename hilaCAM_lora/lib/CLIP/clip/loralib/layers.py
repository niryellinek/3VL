    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                attention_probs_backwards_hook=attention_probs_backwards_hook) #[hilaCAM added hooks]
        else:
            return multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias, self.in_proj_weight_lora_A, self.in_proj_weight_lora_B, self.scaling,
...
def multi_head_attention_forward(query: Tensor,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            #print(f'NIR q.requires_grad: {q.requires_grad}, k.requires_grad: {k.requires_grad}')
    ...
    #print(f'NIR q.requires_grad: {q.requires_grad}, k.requires_grad: {k.requires_grad}, attn_output_weights.requires_grad: {attn_output_weights.requires_grad}')
    ...
    if attention_probs_forward_hook is not None and attention_probs_backwards_hook is not None:
        ...
        #print(f'NIR attn_output_weights.requires_grad: {attn_output_weights.requires_grad}')
        #NIR: attn_output_weights.requires_grad is False (at the first few layers??) and cause register_hook to crash
