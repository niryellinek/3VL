...
class CocoCaptionsDataset(CocoCaptions):
  ...
  def __getitem__(self, index):
    ...
    return index, full_path, image, target
...
def get_tree_loss(model, tree_criterion, images, captions, dataset_index, tree_func, weights_list):
  ...
  # normalize by the sum of all weights in the batch
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  weight_file = "COCO_contrast_samples_weighting.json"
  ...
  print(f'train with lora_rank: {DT_CLIP_model.lora}, tree_loss_weight: {tree_loss_weight}, get_caption_tree6: flan t5 opposites if exist otherwise co-hyponym. add all shuffuled caption after each branch')
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    for batch_idx, (index, full_path, images, captions) in enumerate(tqdm(data_loader)):
      ...
      #average by num of examples in contrastive batch
