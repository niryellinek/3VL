...
def perturbation_image(clip_model, img, text_inputs, positive_cam_image, negaitive_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            pos_score = positive_similarity[0][0] + negaitive_similarity[0][0]
            neg_score = negaitive_similarity[0][1] + positive_similarity[0][1]
...
def main():
    ...
    print(f'use both positive and negative relevancy maps to get two scores')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_separate_cams_double_scores for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
