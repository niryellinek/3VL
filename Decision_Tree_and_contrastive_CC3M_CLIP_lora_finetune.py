...
from create_coarse_sentences import get_caption_tree6, get_caption_tree6_lemmas, get_caption_tree6_shuffled_nouns_adjectives, get_caption_tree6_shuffle_all, get_caption_tree6_shuffle_random_all_branches, get_caption_tree6_with_all_shuffles, get_caption_tree6_with_noun_adj_shuffle
from create_coarse_sentences import get_caption_tree4, get_caption_tree6_1, get_caption_tree6_2, get_caption_tree8, get_caption_tree6_shuffle_all_branches, get_caption_tree6_shuffle_nouns_all_branches, get_caption_tree6_with_random_shuffle
...
sys.path.insert(0, '/mnt5/nir/dataset/')
from parse_conceptual import ConceptualDS
...
# def fetch_single_image(image_url, preprocess=None, timeout=None, retries=0):
...
# def fetch_images(batch, num_threads, preprocess=None, timeout=None, retries=0):
#     fetch_single_image_with_args = partial(fetch_single_image, preprocess=preprocess, timeout=timeout, retries=retries)
#     with ThreadPoolExecutor(max_workers=num_threads) as executor:
#         batch["image"] = list(executor.map(fetch_single_image_with_args, batch["image_url"]))
#     return batch


# def preprocess_single_image(example):
#     example['text'] = 'My text: ' + example['text']
#     return example

# def load_model(self, base_name="ViT-B/32", lora_r=-1, weight_name=""):

# clip_model, preprocess = clip.load("ViT-B/32", jit=False, lora=-1)

# num_threads = 20
# dset = load_dataset("conceptual_captions", split="validation", streaming=True)
# for example in dset:
#example = dset['train'][0]
  # print(f'dset[0]: {example}')
  # break
#dset = dset.with_format("torch", device=device)
# dset = dset.with_format("torch")
# for example in dset:
#example = dset['train'][0]
  # print(f'dset[0]: {example}')
  # break
# dset = dset.map(fetch_images, batched=True, batch_size=4, fn_kwargs={"num_threads": num_threads, "preprocess": preprocess}, remove_columns=["image_url"])
# for example in dset:
#example = dset['train'][0]
  # print(f'dset[0]: {example}')
  # break
# dset= dset.shuffle(seed=42, buffer_size=4)
# for example in dset:
# #example = dset['train'][0]
#   print(f'dset[0]: {example}')
#   break
...
# dataloader = DataLoader(dset) #batch_size and shuffle made in the dataset?
...
class DecisionTreeClipModel(nn.Module):
  ...
  def get_probs_matrix(self, images, true_label_path, all_tree_nodes):
    ...
    # with autograd.detect_anomaly():
      ...
      # if torch.isnan(exponent_similarity).any():
      #   print(f'exponent_similarity has NaN value: \nexponent_similarity: \n{exponent_similarity}')
      #   print(f'\nsimilarity: \n{similarity}')
      #   print(f'torch.isnan(similarity).any(): {torch.isnan(similarity).any()}')
      ...
      #   print(f'probs has NaN value: \nprobs: \n{probs}')
      #   print(f'\nsoftmax_denominator: \n{softmax_denominator}')
      #   print(f'\nsoftmax_numerator: \n{softmax_numerator}')
      #   print(f'torch.isnan(softmax_denominator).any(): {torch.isnan(softmax_denominator).any()}')
      #   print(f'torch.isnan(softmax_numerator).any(): {torch.isnan(softmax_numerator).any()}')
      #   print(f'\nsoftmax_numerator.grad_fn: {softmax_numerator.grad_fn}\n')
      #   print(f'\nsoftmax_denominator.grad_fn: {softmax_denominator.grad_fn}\n')
      #   exit(0)
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  #checkpoint trained with CC3M LR_3e-6 LoRA rank = 1, checkpoint every 800 batches get_caption_tree6_shuffled_nouns_adjectives
  # PATH = '/mnt5/nir/CLIP/interpret/CC3M_DT_0.5_tree6_shuffle_nouns_last_branch_LoRA_1_contrast_LR_3e6_checkpoint_epoch_0000_batch_0024.pt.tar'          

 #checkpoint trained with CC3M LR_3e-6 LoRA rank = 1, checkpoint every 800 batches get_caption_tree6_shuffle_nouns_all_branches
  # PATH = '/mnt5/nir/CLIP/interpret/CC3M_DT_0.5_tree6_shuffle_nouns_all_branches_LoRA_1_contrast_LR_3e6_checkpoint_epoch_0000_batch_0047.pt.tar'          

  # PATH = '/mnt5/nir/CLIP/interpret/CC3M_DT_0.5_caption6_LoRA_1_contrast_LR_3e6_checkpoint_epoch_0000_batch_0002.pt.tar'          
    ...
    for batch_idx, (images, captions, image_names) in enumerate(tqdm(data_loader)):
      ...
      # with autograd.detect_anomaly():
        ...
        with torch.no_grad(): 
          ...
          # shuffle_func = get_caption_tree6_with_random_shuffle
          # shuffle_func = get_caption_tree6
          # shuffle_func = get_caption_tree6_with_all_shuffles
          shuffle_func = get_caption_tree6_with_noun_adj_shuffle
          ...
          while not any(node_texts):
            ...
            node_texts_is_valid = True
          
          if not node_texts_is_valid:
        ...
        # if torch.isnan(log_probabilities).any():
        #   print(f'log_probabilities has NaN value 1: \nlog_probabilities: \n{log_probabilities}')
        #   exit(0)
        ...
        # if torch.isnan(log_probabilities).any():
        #   print(f'log_probabilities has NaN value 2: \nlog_probabilities: \n{log_probabilities}')
        #   exit(0)
        log_probabilities = log_probabilities.float()
        # if torch.isnan(log_probabilities).any():
        #   print(f'log_probabilities has NaN value 3: \nlog_probabilities: \n{log_probabilities}')
        #   exit(0)
        node_num_labels = torch.tensor(true_label_path,device=device, dtype=torch.long)
        ...
        # if torch.isnan(tree_loss).any():
        #   print(f'tree_loss has NaN value: {tree_loss}')
        #   print(f'\nlog_probabilities.shape: \n{log_probabilities.shape}')
        #   print(f'\nnode_num_labels.shape: \n{node_num_labels.shape}')
        #   print(f'\nlog_probabilities: \n{log_probabilities}')
        #   print(f'\nnode_num_labels: \n{node_num_labels}')
        #   exit(0)
        ...
        #   print(f'contrastive_loss has NaN value: \ncontrastive_loss: \n{contrastive_loss}')
        #   exit(0)
        ...
        #   print(f'total_loss has NaN value: 1\ntotal_loss: \n{total_loss}')
        #   exit(0)

        # if math.isnan(total_loss.item()):
        #   print(f'total_loss is NaN value: \ntotal_loss.item(): \n{total_loss.item()}')
        #   exit(0)
        ...
        # backward pass
        # if batch_idx % 100 == 0:
        #   print(f'batch {batch_idx}, tree_loss: {tree_loss}, contrastive_loss: {contrastive_loss}, total_loss: {total_loss}, sum(batch_losses): {sum(batch_losses)}') 

        total_loss = total_loss.to(device)

        # if torch.isnan(total_loss).any():
        #   print(f'total_loss has NaN value: 2\ntotal_loss: \n{total_loss}')
        #   exit(0)
        # if batch_idx % 100 == 0:
        #   print(f'total_loss: {total_loss}') 
          
        #total_loss = total_loss.float()
        total_loss.to(torch.double)
        ...
        #rare, but can happen that tree loss os nan due to empty true_label_path (empty caption?)
        if torch.isnan(total_loss).any():
          print(f'\ntotal_loss has NaN value- do NOT update weights. total_loss: {total_loss}')
          print(f'\noriginal tree caption: {captions[rand_sample_index]}')
          print(f'\n node_texts: \n{node_texts}')                  
          ...
          batch_total_examples += num_examples
        ...
        # print(f'batch {batch_idx}, tree_loss: {tree_loss}, contrastive_loss: {contrastive_loss}') 

        # if math.isnan(sum(batch_losses)):
        #   print(f'sum(batch_losses) is NaN value: \nsum(batch_losses): \n{sum(batch_losses)}')
        #   print(f'batch_losses are:\n\n')
        #   for idx, val in  enumerate(batch_losses):
        #     print(f'idx: {idx}, loss: {val}')
        #   exit(0)
        ...
        if ((batch_idx + 1) % save_every_batches == 0) or (batch_idx + 1 == len(data_loader)):
          ...
          filename = checkpoint_name + f"{completed_epoch:04d}_batch_{checkpoint_index:04d}.pt.tar"
          print(f'saving model state to filename: {filename}')
          torch.save(checkpoint_dict, filename)
          print(f'batch {batch_idx} loss: {sum(batch_losses) / batch_total_examples}, sum(batch_losses): {sum(batch_losses)}, batch_total_examples: {batch_total_examples}, sum(batch_losses): {sum(batch_losses)}') 
          batch_total_examples = 0
          batch_losses = []
...
def check_accuracy(loader, DT_CLIP_model):
    ...
    with torch.no_grad():
        ...
        for images, captions, image_names in tqdm(loader):
            ...
            predictions, labels = DT_CLIP_model.classify_example(image_names[0], images,captions[0], print_all=False, print_incorrect=False)
...
def main():
    ...
    cc3m_root = "/mnt5/nir/dataset/CC3M"
    
    # train_dataset = ConceptualDS(cc3m_root, DT_CLIP_model.preprocess, "train")
    # test_dataset = ConceptualDS(cc3m_root, DT_CLIP_model.preprocess, "val")


    preprocess_train = image_transform(DT_CLIP_model.clip_model.visual.input_resolution, is_train=True, mean=OPENAI_DATASET_MEAN, std=OPENAI_DATASET_STD)
    preprocess_val = image_transform(DT_CLIP_model.clip_model.visual.input_resolution, is_train=False, mean=OPENAI_DATASET_MEAN, std=OPENAI_DATASET_STD)

    train_dataset = ConceptualDS(cc3m_root, preprocess_train, "train")
