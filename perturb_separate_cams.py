...
def perturbation_image(clip_model, img, text_inputs, positive_cam_image, negaitive_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            positive_cam_image = positive_cam_image * (-1)
            negaitive_cam_image = negaitive_cam_image * (-1)

        
        text_features = clip_model.encode_text(text_inputs)
        text_features /= text_features.norm(dim=-1, keepdim=True)

        total_num_patches = positive_cam_image.shape[0]
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, positive_top_patches_indices = positive_cam_image.topk(k=num_top_patches, dim=-1)
            ...
            _, negaitive_top_patches_indices = negaitive_cam_image.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
            negaitive_top_patches_indices = negaitive_top_patches_indices.cpu().data.numpy()
            negaitive_top_patches_indices.sort()
            ...
            negaitive_image_features = clip_model.visual.forward_patches(img,negaitive_top_patches_indices)
            
            #normalize image_features and text_features
            negaitive_image_features /= negaitive_image_features.norm(dim=-1, keepdim=True)

            
            positive_similarity = 100.0 * positive_image_features @ text_features.T
            negaitive_similarity = 100.0 * negaitive_image_features @ text_features.T
            ...
            pos_score = positive_similarity[0][0]
            neg_score = negaitive_similarity[0][1]
...
def perturbation_image_text(clip_model, img, text_inputs, positive_cam_image, negative_cam_image, positive_cam_text, negative_cam_text, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            positive_cam_image = positive_cam_image * (-1)
            negative_cam_image = negative_cam_image * (-1)
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, negative_top_patches_indices = negative_cam_image.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
            ...
            #print(f'\ntop_positive_text_indices: {top_positive_text_indices}')
            #print(f'\ntop_negative_text_indices: {top_negative_text_indices}')

            #top_positive_text_indices = top_positive_text_indices.cpu().numpy()
            #top_negative_text_indices = top_negative_text_indices.cpu().numpy()
            ...
            #print(f'\ncurr_num_positive_tokens: {curr_num_positive_tokens}, curr_num_negative_tokens: {curr_num_negative_tokens}')
            ...
            positive_similarity = 100.0 * positive_image_features @ text_features.T
            negaitive_similarity = 100.0 * negative_image_features @ text_features.T
...
def main():
    ...
    print(f'use separate relevancy maps with text normalization and removal')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_separate_cams for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    pert_acc = perturbation_image_text(clip_model, img, tokenized_text, positive_cam_image, negative_cam_image, positive_cam_text, nagetive_cam_text, pert_steps, pert_acc, is_positive_pert)   

                    #pert_acc = perturbation_image(clip_model, img, tokenized_text, positive_cam_image, negative_cam_image, pert_steps, pert_acc, is_positive_pert)   
