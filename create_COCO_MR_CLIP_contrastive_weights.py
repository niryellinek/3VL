...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, eta, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  DT_CLIP_model.train()
  loaded_epoch = 0
  #loss = None
    ...
    for batch_idx, (index, full_path, images, captions) in enumerate(tqdm(data_loader)):
      ...
      with torch.no_grad(): 
        ...
        if ((batch_idx + 1) % loss_save_every == 0) or (batch_idx + 1 == len(data_loader)):
          with open(f"COCO_samples_contrast_loss_batch_{batch_idx + 1}.json", "w") as fp:
