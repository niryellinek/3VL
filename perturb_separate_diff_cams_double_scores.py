...
def perturbation_image(clip_model, img, text_inputs, positive_diff_cam_image, negaitive_diff_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            positive_diff_cam_image = positive_diff_cam_image * (-1)
            negaitive_diff_cam_image = negaitive_diff_cam_image * (-1)
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, negaitive_top_patches_indices = negaitive_diff_cam_image.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
...
def main():
    ...
    print(f'use both positive - negative and negative - positive relevancy maps to get two scores')
    
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_separate_diff_cams_double_scores for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    positive_diff_cam = positive_cam_image - negative_cam_image
                    negative_diff_cam = negative_cam_image - positive_cam_image
