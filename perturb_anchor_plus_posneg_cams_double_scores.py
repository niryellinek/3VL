...
def perturbation_image(clip_model, img, text_inputs, positive_anc_cam_image, negative_anc_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            positive_anc_cam_image = positive_anc_cam_image * (-1)
            negative_anc_cam_image = negative_anc_cam_image * (-1)

        
        text_features = clip_model.encode_text(text_inputs)
        text_features /= text_features.norm(dim=-1, keepdim=True)

        total_num_patches = positive_anc_cam_image.shape[0]
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, positive_top_patches_indices = positive_anc_cam_image.topk(k=num_top_patches, dim=-1)
            ...
            _, negative_top_patches_indices = negative_anc_cam_image.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
...
def main():
    ...
    print(f'use both anchor plus relevancy maps (positive + ancor, negative + ancor) to get two scores')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_anchor_plus_posneg_cams_double_scores for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    positive_anc_cam = positive_cam_image + ancor_cam_image
                    negative_anc_cam = negative_cam_image + ancor_cam_image
                    
                    positive_anc_cam = (positive_anc_cam - positive_anc_cam.min()) / (positive_anc_cam.max() - positive_anc_cam.min())
                    negative_anc_cam = (negative_anc_cam - negative_anc_cam.min()) / (negative_anc_cam.max() - negative_anc_cam.min())
                   
                    pert_acc = perturbation_image(clip_model, img, tokenized_text, positive_anc_cam, negative_anc_cam, pert_steps, pert_acc, is_positive_pert)   
