...
def perturbation_image(clip_model, img, text_inputs, cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        ...
        text_features = get_text_features(clip_model, text_inputs)
...
def main():
    ...
    #print(f'get relevancy map from {relevancy_map_str} text')
    print(f'get relevancy map from higher score text diff')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_diff_cam_by_score test for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    #print(f'got score - pos_score: {pos_score}, neg_score: {neg_score}')

                    positive_cam_image = R_image[0]
                    negative_cam_image = R_image[1]

                    #get diff by high score positive - negative if pos_score>=neg_score else negative - positive 
