...
def perturbation_image(clip_model, img, text_inputs, positive_cam_image, negative_cam_image, ancor_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            positive_cam_image = positive_cam_image * (-1)
            negative_cam_image = negative_cam_image * (-1)
            ancor_cam_image = ancor_cam_image * (-1)
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, ancor_top_patches_indices = ancor_cam_image.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
            ancor_top_patches_indices = ancor_top_patches_indices.cpu().data.numpy()
            ancor_top_patches_indices.sort()
            ...
            ancor_image_features = clip_model.visual.forward_patches(img,ancor_top_patches_indices)
            ...
            ancor_image_features /= ancor_image_features.norm(dim=-1, keepdim=True)
            ...
            ancor_similarity = 100.0 * ancor_image_features @ text_features.T

            similarity = positive_similarity + negative_similarity + ancor_similarity
...
def main():
    ...
    print(f'use anchor, positve and negative relevancy map to get 3 scores')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_anchor_pos_neg_triple_scores for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    pert_acc = perturbation_image(clip_model, img, tokenized_text, positive_cam_image, negative_cam_image, ancor_cam_image, pert_steps, pert_acc, is_positive_pert)   
