...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  non_valid_samples = torch.zeros_like(weighting)
  epoch_total_loss = torch.zeros_like(weighting)
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    for batch_idx, (index, images, captions, image_names) in enumerate(tqdm(data_loader)):
      ...
      with torch.no_grad(): 
        ...
        # epoch_total_loss[index] = contrastive_loss.item()
        epoch_total_loss[index] = contrastive_loss
        ...
        # loss update
        if ((batch_idx + 1) % loss_save_every == 0) or (batch_idx + 1 == len(data_loader)):
          with open(f"CC3M_samples_loss_batch_{batch_idx + 1}.json", "w") as fp:
            json.dump(epoch_total_loss.cpu().numpy().tolist(), fp)
        #     optimizer.step()
        #     optimizer.zero_grad()
       
      
    completed_epoch = epoch + 1
    #finished 1 epoch with total_examples:2808715
    print(f'finished 1 epoch with total_examples:{total_examples}')
    weight_file = "CC3M_samples_weighting.json"
    print(f'calc weighting and save to file')
    eta = 0.01
    weighting = update_weighting(epoch_total_loss, eta)  

    with open(weight_file, "w") as f:
      json.dump(weighting.cpu().numpy().tolist(), f)
