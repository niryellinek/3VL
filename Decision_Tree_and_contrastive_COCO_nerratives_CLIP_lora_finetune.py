...
class CocoCaptionsDataset(CocoCaptions):
  ...
  def __init__(self, root, annFile, nerrativesFile, transform):
    super().__init__(root, annFile, transform)
    
    self.nerratives_dict = self._parse_nerratives(nerrativesFile)

  def _parse_nerratives(self, nerrativesFile):
    if not nerrativesFile:
      return None

    nerratives_dict = dict()
    
    with open(nerrativesFile, 'rb') as f:
      for line in f:
        line_dct = json.loads(line)
        nerratives_dict[int(line_dct['image_id'])] = line_dct['caption']
        # print(f'_parse_nerratives line_dct: \n{line_dct}')
      
      # exit(0)
    
    return nerratives_dict

  def _load_image(self, id: int) -> Image.Image:
    path = self.coco.loadImgs(id)[0]["file_name"]
    # print(f'_load_image path: {path}')
    full_path = os.path.join(self.root, path)
    # print(f'_load_image full_path: {full_path}')
    return Image.open(full_path).convert("RGB") , full_path

  def __getitem__(self, index):

    img_id = self.ids[index]
    # print(f'__getitem__ index: {index}, img_id: {img_id}')
    image , full_path = self._load_image(img_id)
    # print(f'__getitem__ full_path: {full_path}')
    #target contains 5 captions per image - use only the first caption
    target = self._load_target(img_id)[0]
    # print(f'__getitem__ target: {target}')
    nerrative = self.nerratives_dict[img_id]
    # print(f'__getitem__ nerrative: \n{nerrative}')

    if self.transforms is not None:
      image, target = self.transforms(image, target)
      # image, nerrative = self.transforms(image, nerrative)

    # return full_path, image, target
    return full_path, image, target, nerrative
...
class DecisionTreeClipModel(nn.Module):
  ...
  def get_encoded_prompts(self, node_names):
    ...
    tokenized_prompts = torch.cat([clip.tokenize(p,truncate=True) for p in prompts]).to(device=device)
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  # print(f'train with lora_rank: {DT_CLIP_model.lora}, tree_loss_weight: {tree_loss_weight}, get_caption_tree6_shuffle_all_branches: flan t5 opposites if exist otherwise co-hyponym. add all shuffuled caption after each branch')
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    for batch_idx, (full_path, images, coco_captions, nerratives) in enumerate(tqdm(data_loader)):
      ...
      #TODO: try using also nerratives for contrastive loss
      prompts = coco_captions
...
def main():
    ...
    train_dataset = CocoCaptionsDataset(root="/mnt5/yoavkurtz/datasets/coco2017/train2017", annFile = "/mnt5/yoavkurtz/datasets/coco2017/annotations/captions_train2017.json", 
    nerrativesFile = "/mnt5/nir/CLIP/DATASET/coco_train_captions.jsonl", transform=DT_CLIP_model.preprocess)

    test_dataset = CocoCaptionsDataset(root="/mnt5/yoavkurtz/datasets/coco2017/val2017", annFile = "/mnt5/yoavkurtz/datasets/coco2017/annotations/captions_val2017.json", nerrativesFile=None, transform=DT_CLIP_model.preprocess)
