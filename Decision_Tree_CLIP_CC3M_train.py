...
from create_coarse_sentences import get_caption_tree, Node
#from lora.
#import datasets
...
class DecisionTreeClipModel(nn.Module):
  ...
  def get_encoded_prompts(self, all_nodes):
  ...
  def classify_example(self, x, caption, print_path=False, break_on_incorrect=False):
    
    root, true_label_path, all_tree_nodes = get_caption_tree(caption)
    
    #curr_path_node = root
    #node_path_list = []

    #children = curr_path_node.children
    #if print_path:
    ...
    if print_path:
      ...
      for i, predicted_node_num in enumerate(predictions):
        ...
        if not is_correct:
          ...
          #  print(f'\nnode.node_num: {node.node_num}, node.prompt: {node.prompt}')

          if break_on_incorrect:
            #additional_predicted_caption = expand_caption(predicted_caption, correct_caption)
            #print(f'\nI thought it was a photo of {caption}')
            #print(f'because there is {:.2f}% probability for having {additional_predicted_caption} in the photo\n')

            #print(f'I thought it wasn't a photo of {correct_caption} because\n')
            #additional_correct_caption = expand_caption(correct_caption)
  ...
  def get_probs_matrix(self, images, true_label_path, all_tree_nodes):
    ...
    self.encoded_prompts = self.get_encoded_prompts(all_tree_nodes)
...
def DT_CLIP_CC3M_train(DT_CLIP_model, num_epochs, data_loader, criterion, optimizer, save_frequency=5, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  DT_CLIP_model.train()
  loaded_epoch = 0
  #loss = None
  ...
  for epoch in range(loaded_epoch,num_epochs):
    print(f'start DT_CLIP_CC3M_train epoch#: {epoch+1}')

    losses = []
    total_examples = 0

    for batch_idx, (images, captions) in enumerate(tqdm(data_loader)):
      ...
      #node_id_labels = caption_tree.true_label_path

      log_probabilities = DT_CLIP_model(images,true_label_path, all_tree_nodes)
      node_num_labels = torch.tensor(true_label_path,device=device)

      loss = criterion(log_probabilities, node_num_labels)
      

      
      losses.append(loss.item())
      num_examples = len(node_num_labels)
...
def check_accuracy(loader, DT_CLIP_model):
    ...
    with torch.no_grad():
        ...
        for images, captions in tqdm(loader):
            ...
            predictions, labels = DT_CLIP_model.classify_example(images,captions[0][0], print_path=True, break_on_incorrect=True)
            #label = labels[0].data
...
def main():
    ...
    train_dataset = CocoCaptions(root="/mnt5/yoavkurtz/datasets/coco2017/train2017", annFile = "/mnt5/yoavkurtz/datasets/coco2017/annotations/captions_train2017.json", transform=preprocess)
    test_dataset = CocoCaptions(root="/mnt5/yoavkurtz/datasets/coco2017/val2017", annFile = "/mnt5/yoavkurtz/datasets/coco2017/annotations/captions_val2017.json", transform=preprocess)
