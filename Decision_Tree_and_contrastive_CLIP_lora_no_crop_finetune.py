...
from create_coarse_sentences import get_caption_tree6_gen_si
...
# prompt_templates = ['itap of a {}.', 'a bad photo of the {}.',]

# prompt_templates = ['a photo of {}.',]

def encode_80_prompts(model, classnames, templates):
    with torch.no_grad():
      ...
      for classname in classnames:
          ...
          encoded_prompts.append(class_embedding)
      encoded_prompts = torch.stack(encoded_prompts, dim=1).cuda()
...
class DecisionTreeClipModel(nn.Module):
  def __init__(self, lora=-1, checkpoint_file=""):
    ...
    print(f'new self.preprocess is: {self.preprocess}')
  ...
  def get_encoded_prompts(self, node_names):
    ...
    return encode_80_prompts(self.clip_model, node_names, prompt_templates)
  ...
  def get_cosine_similarity(self, x, encoded_prompts):
    ...
    #don't need to transose with 80 prompts (already transposed)
    similarity = 100.0 * image_features @ encoded_prompts #batch_size X num nodes
...
def DT_Contrastive_CLIP_train(DT_CLIP_model, num_epochs, data_loader, tree_criterion, img_criterion, txt_criterion, optimizer, save_freq=5, update_weights_freq=1, checkpoint_name='checkpoint_DT_CLIP_epoch#'):
  ...
  PATH = '/mnt5/nir/CLIP/interpret/COCO_DT_tree_caption6_no_crop_LoRA_1_LR_3e6_checkpoint_epoch_0009.pt.tar'
...
#   print(f'train with lora_rank: {DT_CLIP_model.lora}, tree_loss_weight: {tree_loss_weight}, get_caption_tree6_shuffle_all_branches: rule based -> flan t5 opposites if exist otherwise co-hyponym. add all shuffuled caption after each branch')
  ...
  for epoch in range(loaded_epoch,num_epochs):
    ...
    #   shuffle_func = get_caption_tree6_shuffle_all_branches
      ...
      # logit_scale = DT_CLIP_model.clip_model.logit_scale.exp()
      # text_features = DT_CLIP_model.clip_model.encode_text(tokenized_prompts)
      # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
      # image_features = DT_CLIP_model.clip_model.encode_image(images)
      # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
      ...
      text_features = encode_80_prompts(DT_CLIP_model.clip_model, captions, prompt_templates)
      image_features = DT_CLIP_model.clip_model.encode_image(images)
      image_features = image_features / image_features.norm(dim=-1, keepdim=True)
