...
def perturbation_image(clip_model, img, text_inputs, high_score_cam, ancor_cam, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        if is_positive_pert:
            high_score_cam = high_score_cam * (-1)
            ancor_cam = ancor_cam * (-1)

        
        text_features = clip_model.encode_text(text_inputs)
        text_features /= text_features.norm(dim=-1, keepdim=True)

        total_num_patches = high_score_cam.shape[0]
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            _, high_score_top_patches_indices = high_score_cam.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
            high_score_top_patches_indices = high_score_top_patches_indices.cpu().data.numpy()
            high_score_top_patches_indices.sort()

            _, anc_top_patches_indices = ancor_cam.topk(k=num_top_patches, dim=-1)
            #print(f'top_patches_indices: {top_patches_indices}')
            anc_top_patches_indices = anc_top_patches_indices.cpu().data.numpy()
            anc_top_patches_indices.sort()

            
            high_score_image_features = clip_model.visual.forward_patches(img,high_score_top_patches_indices)
            anc_image_features = clip_model.visual.forward_patches(img,anc_top_patches_indices)

            #normalize image_features
            high_score_image_features /= high_score_image_features.norm(dim=-1, keepdim=True)
            anc_image_features /= anc_image_features.norm(dim=-1, keepdim=True)

            
            high_score_similarity = 100.0 * high_score_image_features @ text_features.T
            anc_similarity = 100.0 * anc_image_features @ text_features.T


            similarity = high_score_similarity + anc_similarity
...
def main():
    ...
    print(f'use both anchor and high score between positive and negative to get two scores')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_anchor_high_score_posneg_cams_double_scores for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
            ...
            for name in d.data:
                ...
                for index, batch in enumerate(iterator):
                    ...
                    ################
                    with torch.no_grad():
                        text_features = get_text_features(clip_model, tokenized_text)
                        image_features = clip_model.encode_image(img)

                        pos_score, neg_score = get_positve_negative_scores(text_features, image_features)
                    
                    
                    cam_image = negative_cam_image if neg_score > pos_score else positive_cam_image
                                        
                    cam_image = (cam_image - cam_image.min()) / (cam_image.max() - cam_image.min())
                                        
                    ancor_cam_image = (ancor_cam_image - ancor_cam_image.min()) / (ancor_cam_image.max() - ancor_cam_image.min())


                    
                    pert_acc = perturbation_image(clip_model, img, tokenized_text, cam_image, ancor_cam_image, pert_steps, pert_acc, is_positive_pert)   
