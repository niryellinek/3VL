...
def perturbation_image(clip_model, img, text_inputs, positive_cam_image, negaitive_cam_image, pert_steps, pert_acc, is_positive_pert=False):
    with torch.no_grad():
        ...
        for step_idx, step in enumerate(pert_steps):
            ...
            #use cross scores (negative from positive_similarity and vice versa)
            pos_score = negaitive_similarity[0][0]
...
def main():
    ...
    print(f'use separate cross relevancy maps (positive for negative text and vice versa)')
    for data_type in TYPES:
        for is_positive_pert in positive_negative_pert_list:
        
            print(f'starting perturb_separate_cross_cams for data_type: {data_type} with is_positive_pert: {is_positive_pert}')
